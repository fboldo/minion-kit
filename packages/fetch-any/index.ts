...
export interface FetchAnyRequest {
	url: string;
	method: string;
...
export interface FetchAnyResult {
	status: number;
	headers: Record<string, string>;
	contentType: string;
	body: string;
...
/** Defines an extra CLI option that a plugin contributes to `fetch-any --help`. */
...
export interface FetchAnyPlugin {
	...
	preProcess?: (
		request: FetchAnyRequest,
		opts: Record<string, unknown>,
	) => Promise<FetchAnyRequest>;
	/** Transform the result after fetch */
	postProcess?: (
		result: FetchAnyResult,
		opts: Record<string, unknown>,
	) => Promise<FetchAnyResult>;
}
...
export function createPlugin(def: FetchAnyPlugin): FetchAnyPlugin {
	return def;
}
